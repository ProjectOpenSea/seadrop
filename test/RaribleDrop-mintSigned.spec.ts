...
import type { SignedMintValidationParamsStruct } from "../typechain-types/src/ERC721RaribleDrop";
import type { MintParamsStruct } from "../typechain-types/src/RaribleDrop";
import type { Wallet } from "ethers";

describe(`RaribleDrop - Mint Signed (v${VERSION})`, function () {
  ...
  before(async () => {
    ...
    eip712Domain = {
      ...
      verifyingContract: raribleDrop.address,
    };
    eip712Types = {
      SignedMint: [
  ...
  it("Should not mint a signed mint with different params", async () => {
    ...
    const value = BigNumber.from(mintParams.mintPrice).mul(3);
    await expect(
      raribleDrop.connect(payer).mintSigned(
        token.address,
        feeRecipient.address,
    ...
    await token
      .connect(admin)
      .updateAllowedFeeRecipient(raribleDrop.address, payer.address, true);
    await token.updatePayer(raribleDrop.address, payer.address, true);
    await expect(
      raribleDrop.connect(payer).mintSigned(
        token.address,
    ...
    const token2 = await ERC721PartnerRaribleDrop.deploy("", "", admin.address, [
      raribleDrop.address,
    ]);
    await token2.setMaxSupply(100);
    await token2.updateCreatorPayoutAddress(raribleDrop.address, creator.address);
    await token2
      .connect(admin)
      .updateAllowedFeeRecipient(raribleDrop.address, feeRecipient.address, true);
    ...
    expect(
      await raribleDrop.getSignedMintValidationParams(
        token.address,
        signer2.address
      )
    ...
    await expect(
      raribleDrop.connect(minter).mintSigned(
    ...
    expect(
      await raribleDrop.getSignedMintValidationParams(token.address, signer.address)
    ).to.deep.eq([BigNumber.from(0), 0, 0, 0, 0, 0, 0]);
  ...
  it("Should not update SignedMintValidationParams with invalid fee bps", async () => {
    await expect(
      token
        .connect(admin)
        .updateSignedMintValidationParams(raribleDrop.address, signer.address, {
          ...signedMintValidationParams,
          minFeeBps: 11_000,
        })
